...
namespace WpfApplication.Utilities
{
    public class Print
    {
        ...
        public static void CreateDocumentFromTemplateWithFormat(Facture f, string template)
        {
            ...
            document.Replace("client_npa", f.Client.Localite.ToString(), true, true);
            document.Replace("client_localite", "", true, true);
            document.Replace("facture_date", f.DateFacture.ToString("dd.MM.yyyy"), true, true);
            document.Replace("facture_num", f.NumFacture, true, true);
            ...
            for (int r = 0; r < listLignesFactures.Count; r++)
            {
                ...
                for (int c = 0; c < 5; c++)
                {
                    ...
                    Paragraph p2 = DataRow.Cells[c].AddParagraph();
                    TextRange TR2 = null;
                    decimal total = lf.PrixUntaire * lf.Quantite;
                    switch (c)
                    {
                        case 0:
                        ...
                        case 2:
                            ...
                            break;
                        case 3:
                            TR2 = p2.AppendText(lf.PrixUntaire.ToString());
                            break;
                        case 4:
            ...
            document.SaveToFile($"tmp{f.Id}.pdf", FileFormat.PDF);
